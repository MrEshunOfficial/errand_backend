...
import { FileUploadController } from "./filupload.controller.js";
...
export const getProfile = asyncHandler(
  async (req: AuthenticatedRequest, res: Response<AuthResponse>) => {
...
export const updateProfile = asyncHandler(
  async (
    ...
    // Validate updates
    const invalidUpdates = validateUpdates(
      Object.keys(updates),
      allowedUpdates
    );
    ...
    res
      .status(200)
      .json(
        createSuccessResponse(user, profile, "Profile updated successfully")
      );
  }
);

export const updateProfileRole = asyncHandler(
  async (
    ...
    res
      ...
      .json(
        createSuccessResponse(
          user,
          profile,
          `Profile role updated to ${role} successfully`
        )
      );
  }
);

export const updateProfileLocation = asyncHandler(
  async (
    ...
    if (!location?.ghanaPostGPS) {
      return res
        .status(400)
        .json({ message: "Ghana Post GPS address is required" });
    }
    ...
    res
      .status(200)
      .json(
        createSuccessResponse(user, profile, "Location updated successfully")
      );
  }
);

export const getProfileCompleteness = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const userId = validateAuth(req, res);
    if (!userId)
      return res
        .status(401)
        .json({ message: "User ID not found in request", completeness: 0 });
...
export const getProfileWithContext = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const userId = validateAuth(req, res);
    if (!userId) return;
    ...
    res.status(200).json(
      createSuccessResponse(
        user,
        profile,
        "Profile context retrieved successfully",
        {
          hasProfile: !!profile,
...
export const updateProfilePreferences = asyncHandler(
  async (
...
export const updateSpecificPreference = asyncHandler(
  async (
...
export const bulkUpdatePreferences = asyncHandler(
  async (
...
export const batchProfileOperations = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const userId = validateAuth(req, res);
    if (!userId) return;
    ...
    res.status(200).json(
      createSuccessResponse(
        user,
        profile,
        "Batch profile data retrieved successfully",
        {
          hasProfile: !!profile,
          profileRole: profile?.role || null,
          completeness: profile?.completeness || 0,
...
export const restoreProfile = asyncHandler(
  async (req: AuthenticatedRequest, res: Response<AuthResponse>) => {
    ...
    if (!profile.isDeleted)
      return res.status(400).json({ message: "Profile is not deleted" });
...
export const updateMarketplaceStatus = asyncHandler(
  async (
    ...
    res
      ...
      .json(
        createSuccessResponse(
          user,
          profile,
          "Marketplace status updated successfully"
        )
      );
  }
);
...
export const updateVerificationStatus = asyncHandler(
  async (
    req: Request<
      {},
      AuthResponse,
      { userId: string; status: VerificationStatus; reason?: string }
    >,
    res: Response<AuthResponse>
  ) => {
    ...
    if (
      !userId ||
      !status ||
      !Object.values(VerificationStatus).includes(status)
    ) {
      return res.status(400).json({
        message: `Invalid data. UserId and valid status (${Object.values(
          VerificationStatus
        ).join(", ")}) are required`,
      });
    }
...
export const updateModerationStatus = asyncHandler(
  async (
    req: Request<
      {},
      AuthResponse,
      { userId: string; status: ModerationStatus; reason?: string }
    >,
    res: Response<AuthResponse>
  ) => {
    const { userId, status, reason } = req.body;
    ...
    if (
      !userId ||
      !status ||
      !Object.values(ModerationStatus).includes(status)
    ) {
      return res.status(400).json({
        message: `Invalid data. UserId and valid status (${Object.values(
          ModerationStatus
    ...
    res
      ...
      .json(
        createSuccessResponse(
          user,
          profile,
          `Moderation status updated to ${status}`
        )
      );
  }
);

export const initiateProfileVerification = asyncHandler(
  async (req: AuthenticatedRequest, res: Response<AuthResponse>) => {
    const userId = validateAuth(req, res);
    if (!userId) return;
    ...
    if (completeness < 80) {
      return res.status(400).json({
        message:
          "Profile must be at least 80% complete to initiate verification",
      });
    }
    ...
    res
      ...
      .json(
        createSuccessResponse(
          user,
          updatedProfile,
          "Profile verification initiated successfully"
        )
      );
  }
);
...
export const getAllProfiles = asyncHandler(
  async (req: Request, res: Response<ProfileListResponse>) => {
...
export const getProfilesByStatus = asyncHandler(
  async (req: Request, res: Response<ProfileListResponse>) => {
    ...
    if (!["active", "inactive", "suspended", "banned"].includes(status)) {
      return res.status(400).json({
        message:
          "Invalid status. Must be one of: active, inactive, suspended, banned",
      } as any);
    }
    ...
    res
      ...
      .json(
        createPaginatedResponse(
          ...
          page,
          limit,
          `Profiles with status '${status}' retrieved successfully`
        )
      );
  }
);

export const getProfilesByVerificationStatus = asyncHandler(
  async (req: Request, res: Response<ProfileListResponse>) => {
    const { status } = req.params;
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 20;
    ...
    if (
      ...
      return res.status(400).json({
        message: `Invalid verification status. Must be one of: ${Object.values(
          VerificationStatus
        ).join(", ")}`,
      } as any);
    }
    ...
    res
      ...
      .json(
        createPaginatedResponse(
          ...
          page,
          limit,
          `Profiles with verification status '${status}' retrieved successfully`
        )
      );
  }
);

export const getProfilesByModerationStatus = asyncHandler(
  async (req: Request, res: Response<ProfileListResponse>) => {
    const { status } = req.params;
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 20;
    ...
    if (!Object.values(ModerationStatus).includes(status as ModerationStatus)) {
      return res.status(400).json({
        message: `Invalid moderation status. Must be one of: ${Object.values(
          ModerationStatus
        ).join(", ")}`,
      } as any);
    }
    ...
    res
      ...
      .json(
        createPaginatedResponse(
          ...
          page,
          limit,
          `Profiles with moderation status '${status}' retrieved successfully`
        )
      );
  }
);

export const getIncompleteProfiles = asyncHandler(
  async (req: Request, res: Response<ProfileListResponse>) => {
    ...
    res
      ...
      .json(
        createPaginatedResponse(
          ...
          page,
          limit,
          `Incomplete profiles (< ${threshold}%) retrieved successfully`
        )
      );
  }
);

export const getMarketplaceActiveProfiles = asyncHandler(
  async (req: Request, res: Response<ProfileListResponse>) => {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 20;
    const skip = (page - 1) * limit;
    ...
    res
      ...
      .json(
        createPaginatedResponse(
          ...
          page,
          limit,
          "Marketplace active profiles retrieved successfully"
        )
      );
  }
);

export const recalculateProfileCompleteness = asyncHandler(
...
export const searchProfiles = asyncHandler(
  async (req: Request, res: Response<ProfileListResponse>) => {
    ...
    if (
      moderationStatus &&
      Object.values(ModerationStatus).includes(
    ...
    if (minCompleteness || maxCompleteness) {
      query.completeness = {};
      if (minCompleteness)
        query.completeness.$gte = parseInt(minCompleteness as string);
      if (maxCompleteness)
        query.completeness.$lte = parseInt(maxCompleteness as string);
    }
    ...
    res
      ...
      .json(
        createPaginatedResponse(
          ...
          "Profiles search completed successfully"
        )
      );
  }
);

export const getProfilesByLocation = asyncHandler(
  async (req: Request, res: Response<ProfileListResponse>) => {
    const {
      latitude,
      longitude,
    ...
    res
      ...
      .json(
        createPaginatedResponse(
          ...
          parseInt(page as string),
          parseInt(limit as string),
          `Profiles within ${radius}km retrieved successfully`
        )
      );
  }
);
...
export const addSocialMediaHandle = asyncHandler(
  async (
    req: Request<{}, AuthResponse, { nameOfSocial: string; userName: string }> &
      AuthenticatedRequest,
    res: Response<AuthResponse>
  ) => {
    const userId = validateAuth(req, res);
    ...
    if (!nameOfSocial || !userName) {
      return res
        .status(400)
        .json({ message: "Social media name and username are required" });
    }
...
export const removeSocialMediaHandle = asyncHandler(
  async (
    req: Request<{ handleId: string }> & AuthenticatedRequest,
    res: Response<AuthResponse>
  ) => {
    const userId = validateAuth(req, res);
    if (!userId) return;
    ...
    res
      ...
      .json(
        createSuccessResponse(
          user,
          profile,
          "Social media handle removed successfully"
        )
      );
  }
);
...
export const moderateProfileContent = asyncHandler(
  async (
    req: Request<
      {},
      AuthResponse,
    ...
    const updatedProfile = await profile.updateModeration(
      status,
      moderatedBy,
      notes
    );
    ...
    res
      ...
      .json(
        createSuccessResponse(
          user,
          updatedProfile,
          "Profile moderation updated successfully"
        )
      );
  }
);

export const getPendingModerationProfiles = asyncHandler(
  async (req: Request, res: Response<ProfileListResponse>) => {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 20;
    const skip = (page - 1) * limit;
    ...
    res
      ...
      .json(
        createPaginatedResponse(
          ...
          page,
          limit,
          "Pending moderation profiles retrieved successfully"
        )
      );
  }
);
...
export const exportProfileData = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const userId = validateAuth(req, res);
    if (!userId) return;
...
export const getProfileActivitySummary = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const userId = validateAuth(req, res);
    if (!userId) return;
...
export const updateProfilePicture = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    req.body.file = req.body.profilePicture;
    req.params.entityType = "profile";
