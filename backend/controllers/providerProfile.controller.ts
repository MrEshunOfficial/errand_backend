...
export class ProviderProfileController {
  ...
  private static readonly POPULATE_OPTIONS = [
    { path: "profileId", select: "userId role bio location contactDetails" },
    {
      ...
      select: "title description status images slug",
  ...
  private static async authenticateAndGetProfile(
    ...
    const userProfile = await ProviderProfileController.getUserProfile(
      userId,
      res
    );
  ...
  private static async safePopulate(
    document: any,
    populateOptions = ProviderProfileController.POPULATE_OPTIONS
  ) {
    try {
  ...
  private static sendSuccess(
    ...
    if (data?.providerProfile !== undefined)
      response.providerProfile = data.providerProfile;
  ...
  static async createProviderProfile(
    ...
    try {
      ...
      const savedProfile = await new ProviderProfileModel(
        providerProfileData
      ).save();
      await ProviderProfileController.safePopulate(savedProfile);
  ...
  static async getMyProviderProfile(
    ...
    try {
      const providerProfile =
        await ProviderProfileController.authenticateAndGetProfile(req, res);
      if (!providerProfile) return;

      await ProviderProfileController.safePopulate(providerProfile);
      res.status(200).json({
  ...
  static async updateMyProviderProfile(
    ...
    try {
      ...
      await ProviderProfileController.safePopulate(updatedProfile);
  ...
  static async getProviderProfileById(
    ...
    try {
      ...
      if (!validateObjectId(id)) {
        res
          .status(400)
          .json({ success: false, message: "Invalid provider profile ID" });
        return;
      }

      const providerProfile =
        await ProviderProfileController.getProviderProfile(
    ...
    try {
      ...
      if (available !== undefined)
        filter.isCurrentlyAvailable =
          (available as unknown as string) === "true";
      ...
      const [profiles, total] = await Promise.all([
        ProviderProfileModel.find(filter)
          .populate(ProviderProfileController.POPULATE_OPTIONS)
  ...
  static async getAvailableProviders(
    ...
    try {
      const radius = req.query.serviceRadius
        ? Number(req.query.serviceRadius)
        : undefined;
      const providers = await ProviderProfileModel.findAvailableProviders(
        radius
      );
      this.sendSuccess(
        res,
  ...
  static async getTopRatedProviders(
    ...
    try {
      ...
      const providers = await ProviderProfileModel.findTopRatedProviders(
        limitNumber
      );
      this.sendSuccess(
        res,
  ...
  static async getHighRiskProviders(
    ...
    try {
      ...
      this.sendSuccess(
        res,
        "High-risk providers retrieved successfully",
        providers
      );
    } catch (error) {
  ...
  static async updateOperationalStatus(
    ...
    try {
      ...
      if (
        !validateObjectId(id) ||
        !Object.values(ProviderOperationalStatus).includes(status)
      ) {
        res.status(400).json({
          success: false,
          message: !validateObjectId(id)
   ...
   */
  private static async toggleAvailabilityLogic(
    providerProfile: any,
    res: Response
  ) {
    ...
    this.sendSuccess(
      ...
      `Provider availability ${
        providerProfile.isCurrentlyAvailable ? "enabled" : "disabled"
      } successfully`
    );
  }
  ...
  static async toggleMyAvailability(
    ...
    try {
      const providerProfile =
        await ProviderProfileController.authenticateAndGetProfile(req, res);
      if (!providerProfile) return;
      await ProviderProfileController.toggleAvailabilityLogic(
        providerProfile,
        res
      );
    } catch (error) {
  ...
  static async updatePerformanceMetrics(
    req: Request<
      { id: string },
      ApiResponse,
      Partial<ProviderProfile["performanceMetrics"]>
    >,
    res: Response<ApiResponse>,
    next: NextFunction
  ): Promise<void> {
  ...
  static async updateMyWorkingHours(
    req: AuthenticatedRequest &
      Request<
        {},
        ApiResponse,
        { day: string; hours: { start: string; end: string } }
      >,
    res: Response<ApiResponse>,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      await ProviderProfileController.updateWorkingHoursLogic(
        providerProfile,
        day,
        hours,
        res
    ...
    try {
      ...
      if (!validateObjectId(id) || daysFromNow < 1 || daysFromNow > 365) {
        res.status(400).json({
          ...
          message: !validateObjectId(id)
            ? "Invalid provider profile ID"
            : "Days from now must be between 1 and 365",
        });
        return;
      }
  ...
  private static async getProvidersByCriteria(
    ...
    const providers = await ProviderProfileModel.find({
      ...
      .populate([
        ...ProviderProfileController.POPULATE_OPTIONS,
        ...populateOptions,
      ])
      .sort({ updatedAt: -1 });
  ...
  static async getProvidersByStatus(
    ...
    try {
      ...
      if (!Object.values(ProviderOperationalStatus).includes(status)) {
        res
          .status(400)
          .json({ success: false, message: "Invalid operational status" });
        return;
      }

      await ProviderProfileController.getProvidersByCriteria(
  ...
  static async getProvidersByRiskLevel(
    ...
    try {
      ...
      this.sendSuccess(
        res,
        `Providers with ${riskLevel} risk level retrieved successfully`,
        providers
      );
    } catch (error) {
    ...
    try {
      ...
      if (!userId || !providerIds?.length) {
        res.status(400).json({
          ...
          message: !userId
            ? "Authentication required"
            : "Provider IDs array is required",
        });
        return;
      }
      ...
      const results = await Promise.allSettled(
        providerIds.map(async (id) => {
          ...
          if (!providerProfile)
            return { id, success: false, reason: "Provider not found" };
  ...
  static async getProviderStatistics(
    ...
    try {
      ...
      const [
        ...
        ProviderProfileModel.countDocuments({
          ...baseFilter,
          operationalStatus: ProviderOperationalStatus.SUSPENDED,
        }),
        ProviderProfileModel.countDocuments({
          ...baseFilter,
  ...
  static async getPublicProviderProfile(
    ...
    try {
      ...
      const providerProfile = await ProviderProfileModel.findOne({
        ...
        .select(
          "-riskLevel -lastRiskAssessmentDate -riskAssessedBy -penaltiesCount -lastPenaltyDate -deletedAt -deletedBy"
        )
        .lean()
        .exec();
      ...
      if (!providerProfile) {
        res
          ...
          .json({
            success: false,
            message: "Provider profile not found or not available",
          });
        return;
      }
      ...
      ProviderProfileController.sendSuccess(
        res,
        "Public provider profile retrieved successfully",
        publicProfile
      );
    } catch (error) {
  ...
  static async getPublicProviderProfiles(
    req: Request<
      {},
      ApiResponse<PaginatedResponse<Partial<ProviderProfile>>>,
      {},
      QueryParams & {
        serviceId?: string;
        minRating?: string;
    ...
    try {
      ...
      if (minRating) {
        filter["performanceMetrics.averageRating"] = {
      ...
      if (search) {
        filter.$or = [{ businessName: { $regex: search, $options: "i" } }];
      }
      ...
      const limitNum = Math.max(
        1,
        Math.min(50, parseInt(limit as unknown as string))
      );
      const skip = (pageNum - 1) * limitNum;
      const sortDirection = order === "asc" ? 1 : -1;
      ...
      const publicProfiles: Partial<ProviderProfile>[] = profiles.map(
        (profile) => ({
          _id: profile._id,
      ...
      ProviderProfileController.sendSuccess(
        res,
        "Public provider profiles retrieved successfully",
        {
          data: publicProfiles,
          total,
          page: pageNum,
  ...
  static async searchPublicProviders(
    req: Request<
      {},
      ApiResponse<Partial<ProviderProfile>[]>,
      {},
      {
        lat?: string;
        lng?: string;
    ...
    try {
      ...
      if (lat && lng) {
        ...
        if (isNaN(latitude) || isNaN(longitude) || isNaN(searchRadius)) {
          res
            .status(400)
            .json({ success: false, message: "Invalid location parameters" });
          return;
        }
      }
      ...
      const providers = await ProviderProfileModel.find(filter)
        ...
        .select(
          "businessName serviceOfferings workingHours performanceMetrics isCurrentlyAvailable isAlwaysAvailable requireInitialDeposit percentageDeposit createdAt updatedAt"
        )
        .sort({
      ...
      const publicProviders: Partial<ProviderProfile>[] = providers.map(
        (provider) => ({
          _id: provider._id,
      ...
      ProviderProfileController.sendSuccess(
        res,
        "Public provider search completed successfully",
        publicProviders
      );
    } catch (error) {
