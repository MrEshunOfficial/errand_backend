...
import {
  ...
  SeverityLevel,
  WarningCategory,
} from "../models/warning.model.js";
import {
  WarningResponse,
  UserWarning,
  CreateWarningRequestBody,
  UpdateWarningRequestBody,
} from "../types/warning.types.js";
...
const createPaginatedResponse = (
  ...
  if (includeSummary) {
    response.summary = {
      ...
      resolved: warnings.filter((w) => w.status === WarningStatus.RESOLVED)
        .length,
      expired: warnings.filter((w) => w.status === WarningStatus.EXPIRED)
        .length,
...
const buildQuery = (queryParams: any, baseQuery: any = {}) => {
  ...
  if (status && Object.values(WarningStatus).includes(status))
    query.status = status;
  if (severity && Object.values(SeverityLevel).includes(severity))
    query.severity = severity;
  if (category && Object.values(WarningCategory).includes(category))
...
export const createWarning = asyncHandler(
  async (
    req: Request<{}, WarningResponse, CreateWarningRequestBody> &
    ...
    const { userId, profileId, category, severity, reason, details, ...rest } =
      req.body;
    ...
    if (profile.userId.toString() !== userId.toString()) {
      return res
        .status(400)
        .json({ message: "Profile does not belong to the specified user" });
    }
    ...
    res
      .status(201)
      .json(createWarningResponse(warning, "Warning created successfully"));
  }
);
...
export const getWarning = asyncHandler(
  async (
    ...
    res
      .status(200)
      .json(createWarningResponse(warning, "Warning retrieved successfully"));
  }
);
...
export const updateWarning = asyncHandler(
  async (
    ...
    res
      .status(200)
      .json(
        createWarningResponse(updatedWarning, "Warning updated successfully")
      );
  }
);
  ...
  switch (action) {
    case "acknowledge":
      if (warning.isAcknowledged)
        throw new Error("Warning already acknowledged");
...
export const acknowledgeWarning = asyncHandler(
  async (
    req: Request<{ warningId: string }> & AuthenticatedRequest,
    res: Response<WarningResponse>
  ) => {
    ...
    const warning = await performWarningAction(
    ...
    res
      .status(200)
      .json(
        createWarningResponse(warning, "Warning acknowledged successfully")
      );
  }
);

export const resolveWarning = asyncHandler(
  async (
    req: Request<{ warningId: string }, WarningResponse, { notes?: string }> &
      AuthenticatedRequest,
    res: Response<WarningResponse>
  ) => {
    ...
    res
      .status(200)
      .json(createWarningResponse(warning, "Warning resolved successfully"));
  }
);

export const activateWarning = asyncHandler(
  async (
    req: Request<{ warningId: string }> & AuthenticatedRequest,
    res: Response<WarningResponse>
  ) => {
    ...
    res
      .status(200)
      .json(createWarningResponse(warning, "Warning activated successfully"));
  }
);

export const deactivateWarning = asyncHandler(
  async (
    req: Request<{ warningId: string }> & AuthenticatedRequest,
    res: Response<WarningResponse>
  ) => {
    ...
    res
      .status(200)
      .json(createWarningResponse(warning, "Warning deactivated successfully"));
  }
);

// ===== QUERY OPERATIONS =====

const getWarningsWithPagination = async (
  baseQuery: any,
  req: Request,
  message: string,
  includeSummary = false
...
export const getUserWarnings = asyncHandler(
  async (
...
export const getProfileWarnings = asyncHandler(
  async (
...
export const getAllWarnings = asyncHandler(
  async (req: Request, res: Response<WarningListResponse>) => {
    const result = await getWarningsWithPagination(
      {},
      req,
...
export const getWarningsByCategory = asyncHandler(
  async (
...
export const getWarningsBySeverity = asyncHandler(
  async (
    ...
    if (!Object.values(SeverityLevel).includes(severity as any)) {
      return res.status(400).json({
        message: `Invalid severity. Must be one of: ${Object.values(
          SeverityLevel
        ).join(", ")}`,
      } as any);
    }
...
export const getPendingAcknowledgments = asyncHandler(
  async (req: Request, res: Response<WarningListResponse>) => {
    const result = await getWarningsWithPagination(
      {
        isActive: true,
...
export const getExpiredWarnings = asyncHandler(
  async (req: Request, res: Response<WarningListResponse>) => {
    const { page, limit, skip } = getPagination(req);
    const query = {
      $or: [
    ...
    res
      ...
      .json(
        createPaginatedResponse(
          ...
          page,
          limit,
          "Expired warnings retrieved successfully"
        )
      );
  }
);
...
export const bulkAcknowledgeWarnings = asyncHandler(
  async (
    ...
    if (!Array.isArray(warningIds) || warningIds.length === 0) {
      return res
        .status(400)
        .json({ message: "warningIds array is required and cannot be empty" });
    }
...
export const bulkResolveWarnings = asyncHandler(
  async (
    req: Request<{}, any, { warningIds: string[]; notes?: string }> &
      AuthenticatedRequest,
    res: Response
  ) => {
...
export const expireOldWarnings = asyncHandler(
...
export const getWarningAnalytics = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const [
      ...
      Warning.aggregate<{ _id: string; count: number }>(
        aggregateByField("category")
      ),
      Warning.aggregate<{ _id: string; count: number }>(
        aggregateByField("severity")
      ),
      Warning.aggregate([
        {
          $group: {
            _id: null,
            acknowledged: {
              $sum: { $cond: [{ $ne: ["$acknowledgedBy", null] }, 1, 0] },
            },
            unacknowledged: {
              $sum: { $cond: [{ $eq: ["$acknowledgedBy", null] }, 1, 0] },
            },
          },
        },
      ]),
    ...
    res.status(200).json({
      ...
      data: {
        ...
        acknowledgmentStatus: acknowledgedStats[0] || {
          acknowledged: 0,
          unacknowledged: 0,
        },
...
export const getUserWarningsSummary = asyncHandler(
...
function calculateUserRiskLevel(
  ...
  const severeCount =
    severityBreakdown.find((s) => s._id === SeverityLevel.SEVERE)?.count || 0;
  const majorCount =
    severityBreakdown.find((s) => s._id === SeverityLevel.MAJOR)?.count || 0;

  if (severeCount >= 2 || activeWarnings >= 10) return RiskLevel.CRITICAL;
  if (severeCount >= 1 || majorCount >= 3 || activeWarnings >= 5)
...
const createUtilityResponse = (data: any, message: string) => ({
  message,
  ...data,
});

export const getWarningCategories = asyncHandler(
  async (req: Request, res: Response) => {
    res
      .status(200)
      .json(
...
export const getSeverityLevels = asyncHandler(
  async (req: Request, res: Response) => {
    res
      .status(200)
      .json(
...
export const getWarningStatuses = asyncHandler(
  async (req: Request, res: Response) => {
    res
      .status(200)
      .json(
...
export const cleanupExpiredWarnings = asyncHandler(
  async (req: Request, res: Response) => {
...
export const syncProfileWarningCounts = asyncHandler(
  async (req: Request, res: Response) => {
